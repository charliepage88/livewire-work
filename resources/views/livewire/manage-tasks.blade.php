<div>
    <div x-data="{ is_editing: @entangle('is_editing'), is_deleting: @entangle('is_deleting') }">
        @foreach($tasks as $i => $task)
            <div x-data="{ task: @js($task) }" class="flex flex-col task-row" data-id="{{ $task['id'] }}">
                ...
                <div class="w-full flex mb-3">
                    {{-- is_done --}}
                    @if($task['is_done'])
                        <span class="w-auto inline-block text-white font-bold mr-4" x-show="is_editing !== task.id">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                            </svg>
                        </span>
                    @else
                         <span class="w-auto inline-block text-white font-bold mr-4" x-show="is_editing !== task.id && !task.is_done">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    ...
                    <div x-show="is_editing === task.id" class="flex items-start mr-6">
                        <div class="flex items-center h-8">
                            <input wire:model="tasks.{{ $i }}.is_done" type="checkbox" class="w-8 h-8 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800">
                        </div>
                    </div>

                    {{-- label --}}
                    <span class="w-auto flex-grow inline-block text-white mr-4" x-show="is_editing !== task.id">{{ $task['label'] }}</span>
                    <input x-show="is_editing === task.id" wire:model.lazy="tasks.{{ $i }}.label" type="text" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light mr-6 pr-6" required>

                    {{-- hours --}}
                    <span class="w-auto inline-block text-white mr-4" x-show="is_editing !== task.id">{{ $task['hours'] }} hr</span>
                    <div x-show="is_editing === task.id" class="relative font-white mr-6">
                        ...
                        <input wire:model.lazy="tasks.{{ $i }}.hours" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-12 p-1.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>

                    {{-- is_time_in --}}
                    @if($task['is_time_in'])
                        <span class="w-auto inline-block text-white font-bold" x-show="is_editing !== task.id">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    ...
                    <div x-show="is_editing !== task.id" class="w-48 text-right pl-2">
                ...
                <div x-show="is_editing === task.id" class="w-auto text-center mb-6">
                ...
                <div x-show="is_deleting === task.id" class="w-auto text-center mb-6">
    ...
    @includeWhen($canCreate, 'livewire/components/create-task')
</div>